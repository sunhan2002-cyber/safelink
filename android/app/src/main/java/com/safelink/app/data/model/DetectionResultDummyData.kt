...
object DetectionResultDummyData {
    ...
    val vpCritical = DetectionResult(
        ...
        // 이 시나리오는 전부 키워드 매칭 기반이라 문장 규칙/상황 규칙 근거가 없음(기본값
        // emptyList() 그대로) - 키워드 근거는 위 matchedKeywords 참고. 문장 규칙/상황 규칙
        // 근거 미리보기는 신기훈 5주차 결과물 01번 문서의 실제 검증된 예시 문장 참고.
    )
    ...
    val rsWarning = DetectionResult(
        ...
        appliedComboIds = listOf("COMBO-GENERAL-3CAT")
        // 위와 동일 - 문장 규칙/상황 규칙 근거 없음(emptyList())
    )
