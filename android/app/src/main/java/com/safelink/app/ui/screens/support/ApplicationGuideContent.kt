...
object ApplicationGuideContent {
    ...
    fun forInstitution(institution: InstitutionEntry): ApplicationGuide =
        GUIDES[institution.id] ?: defaultGuideFor(institution)

    /**
     * contact 형식(전화번호/웹주소/안내문구)에 따라 1단계 문구가 달라진다 — 전화번호가 없는
     * 기관(SupportDetailScreen에서도 "전화하기" 버튼을 숨기는 것과 같은 기준)까지 "대표번호로
     * 전화해..."라고 안내하면 실제 화면(전화 버튼 없음)과 어긋난다.
     */
    private fun defaultGuideFor(institution: InstitutionEntry) = ApplicationGuide(
        steps = listOf(
            when {
                institution.phoneOrNull() != null -> "대표번호로 전화해 상담 가능 여부와 준비물을 확인합니다"
                institution.websiteOrNull() != null -> "홈페이지에서 상담 가능 여부와 준비물을 확인합니다"
                else -> "기관에 문의해 상담 가능 여부와 준비물을 확인합니다"
            },
            "아래 서류를 준비합니다",
