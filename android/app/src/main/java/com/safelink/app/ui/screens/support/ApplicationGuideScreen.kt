...
fun ApplicationGuideScreen(navController: NavHostController, institutionId: String) {
    ...
    // 목록에 없는 기관(DEFAULT)은 contact 형식(전화/웹/안내문구)에 따라 1단계 문구가 달라진다 —
    // 전화번호가 없는 기관까지 "전화하여 예약"이라고 안내하면 "기관에 전화하기" 버튼이 없는
    // 화면과 어긋나기 때문(SupportDetailScreen과 같은 기준).
    val guide = remember(institution.id) { ApplicationGuideContent.forInstitution(institution) }
