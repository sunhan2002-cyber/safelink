...
/**
 * AI 보조분석을 쓸지에 대한 사용자 동의 — 실시간 보호(백그라운드)와 직접 분석(붙여넣기·스크린샷)에 함께 적용한다.
 *
 * 예전에는 "백그라운드 AI 보조분석"과 "직접 분석 AI 보조분석"을 따로 받았는데, 사용자 입장에서는 같은 기능이라
 * 설정에 비슷한 스위치가 두 개 있는 게 헷갈렸다. 동의 하나로 합쳤다(예전 직접 분석 동의 값은 쓰지 않는다).
 * 동의하지 않아도 결과 화면의 "AI 보조분석 요청"으로 한 건씩은 직접 받을 수 있다.
 *
 * ── 왜 동의를 따로 받는가 ──────────────────────────────────────────────
...
object AiConsentStore {
    ...
    const val CONSENT_BODY =
        "실시간 보호로 감지한 대화와 직접 분석한 대화 중, 기기 안에서 판단하기 애매한 경우에 한해 " +
            "대화 내용이 AI 제공사(Anthropic)로 전송됩니다.\n\n" +
            "⚠️ 전송되는 내용에 상대방이 보낸 메시지가 포함됩니다. " +
            "전화번호·계좌번호·주민등록번호·카드번호·이메일은 가리고, 링크는 사이트 주소 앞부분(예: naver.com)만 남긴 채 보냅니다. " +
    ...
    /** AI 보조분석을 자동으로 써도 되는지(실시간 보호·직접 분석 공통). 기본값은 꺼짐. */
